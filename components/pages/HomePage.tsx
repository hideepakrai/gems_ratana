...
const Services = ({ section }: { section: any }) => {
  ...
  const heading =
    col.find((b: any) => b.type === "heading")?.text || "Our Bespoke Services";
...
const Collections = ({ section }: { section: any }) => {
  ...
  const col = section.columns?.[0] || [];
  const heading =
    col.find((b: any) => b.type === "heading")?.text || "Our Collections";
    ...
    >
      ...
      >
        ...
        <p className="text-white/80 font-semibold max-w-[540px] mt-3">
          {pBlock?.text ||
            "Crafted with premium Italian velvet and sustainable oak..."}
        </p>
        {button && (
          <Link
      ...
      <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-10 items-center">
        ...
        >
          <h3 className="font-heading md:text-[42px] text-[28px] font-extrabold leading-none">
            {headingBlock?.text ||
              "Premium wood, soft textiles, timeless forms."}
          </h3>
          ...
          <div className="grid gap-3 mt-[18px]">
            {(
              listBlock?.items || ["Solid oak", "Stain-resistant", "Tested"]
            ).map((li: string) => (
              <div
                key={li}
...
const Testimonials = ({ section }: { section: any }) => {
  ...
  const col = section.columns?.[0] || [];
  const heading =
    col.find((b: any) => b.type === "heading")?.text || "What People Say";
...
const Blog = ({ section }: { section: any }) => {
  ...
  const col = section.columns?.[0] || [];
  const heading =
    col.find((b: any) => b.type === "heading")?.text || "The Journal";
  const items = col.find((b: any) => b.type === "cards")?.items || [];
...
const FAQ = ({ section }: { section: any }) => {
  ...
  const col = section.columns?.[0] || [];
  const heading =
    col.find((b: any) => b.type === "heading")?.text ||
